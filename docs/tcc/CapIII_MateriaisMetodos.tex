...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{1.3cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{2.35cm}}
\toprule
\textbf{Código} & \textbf{Descrição} & \textbf{Referência} \\
\midrule
RF01 & Permitir autenticação de usuário para acesso às funcionalidades administrativas do sistema. & Seção \ref{sec:acesso-permissoes}. \\
RF02 & Permitir o cadastro, edição, remoção e consulta de locais físicos do colegiado. & Seção \ref{sec:operacoes-cadastro}. \\
RF03 & Permitir o cadastro, edição, remoção e consulta de leitores RFID, associando cada leitor a um local e a um tipo de operação. & Seção \ref{sec:operacoes-cadastro}. \\
RF04 & Permitir o cadastro, edição, remoção e consulta de itens patrimoniais associados a etiquetas RFID. & Seção \ref{sec:operacoes-cadastro}. \\
RF05 & Permitir o acionamento de janelas de leitura ou auditoria para leitores RFID cadastrados. & Seção \ref{sec:fluxo-eventos-rfid}. \\
RF06 & Receber eventos RFID por interface padronizada, incluindo eventos \textit{motion\_detected} e \textit{tags\_read}. & Seção \ref{sec:fluxo-eventos-rfid}. \\
RF07 & Processar leituras RFID, validar tags, tratar duplicidades e atualizar o local físico dos itens quando aplicável. & Seção \ref{sec:fluxo-eventos-rfid}. \\
RF08 & Registrar histórico operacional das leituras, movimentações, auditorias e alterações relevantes. & Seção \ref{sec:auditoria-inconsistencias-log}. \\
RF09 & Identificar e registrar inconsistências, como local divergente, item não encontrado e tag desconhecida. & Seção \ref{sec:auditoria-inconsistencias-log}. \\
RF10 & Permitir a consulta do inventário, do log operacional, das auditorias e das inconsistências registradas. & Seção \ref{sec:interface-sistema}. \\
RF11 & Permitir o acompanhamento e a resolução de inconsistências registradas no processo de inventário. & Seção \ref{sec:auditoria-inconsistencias-log}. \\
RF12 & Permitir configurações administrativas de usuários e permissões como apoio ao uso operacional do sistema. & Seção \ref{sec:acesso-permissoes}. \\
\bottomrule
\end{tabularx}
...
Os requisitos funcionais foram relacionados às telas, endpoints e fluxos implementados, conforme indicado na própria Tabela \ref{tab:requisitos-funcionais} e discutido no Capítulo IV. A validação funcional experimental concentra-se nos cenários de RFID, auditoria, inconsistências e histórico, apresentados na Tabela \ref{tab:rastreabilidade-validacao}; os demais requisitos são apresentados como evidências de implementação, sem atribuição de validação experimental específica. Com isso, mantém-se a rastreabilidade entre requisitos, modelagem, implementação e validação sem ampliar o escopo das evidências.
...
O diagrama de classes da Figura \ref{fig:diagrama-classes-rfid} apresenta as classes centrais do domínio, suas associações conceituais e os serviços responsáveis pelo processamento dos eventos RFID. As entidades evidenciam a organização de locais, leitores RFID, itens patrimoniais, leituras e inconsistências, enquanto os serviços concentram as operações de acionamento e processamento implementadas no backend.

\input{figs/diagrama_classes_rfid.tex}
...
A validação foi definida como funcional e qualitativa, pois o objetivo foi verificar se os principais fluxos do sistema operam corretamente com o hardware disponível. Para cada cenário, foram definidos uma entrada controlada, o processamento esperado e um critério observável de êxito. Assim, o trabalho foi caracterizado como funcional pela verificação ponta a ponta da leitura da tag, da captura pelo adaptador intermediário, do envio do evento à API REST, do processamento pelo backend, da persistência dos dados, da atualização do inventário e da geração de registros de auditoria. Métricas físicas, como alcance, leitura simultânea, taxa de leitura e influência do ambiente sobre o sinal RFID, foram mantidas fora do escopo experimental. A definição de cenários e resultados esperados aproxima-se de práticas de documentação e execução de testes de software, nas quais casos de teste são estruturados para verificar comportamentos esperados do sistema \cite{iso29119}.
