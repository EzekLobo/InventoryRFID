...
A Figura \ref{fig:arquitetura-visao-geral-rfid} apresenta os componentes principais e suas relações. O sistema web concentra a interação de administradores e usuários com os cadastros, consultas e auditorias. A API REST é o ponto padronizado de entrada para essas requisições e para os eventos RFID. No backend, o \textit{middleware} encaminha os eventos para as regras de inventário, que validam as leituras e registram seus resultados no banco de dados, mantendo a separação entre a interface, o processamento e a persistência.

\input{figs/arquitetura_visao_geral_rfid.tex}

\clearpage
\subsection{Fluxo validado fisicamente}

A Figura \ref{fig:arquitetura-fluxo-validado-rfid} detalha o caminho empregado na validação prática. A tag é aproximada do leitor RFID USB de proximidade, que fornece seu identificador ao comunicador intermediário executado no computador local. Esse comunicador transforma a leitura em uma requisição HTTP do tipo \textit{tags\_read} para a API. O backend então valida a leitura e atualiza os registros de inventário, auditoria e histórico conforme as regras aplicáveis.

\input{figs/arquitetura_fluxo_validado_rfid.tex}

\clearpage
\subsection{Fluxo suportado pela API para dispositivos em rede}

A Figura \ref{fig:arquitetura-fluxo-api-rfid} apresenta o fluxo aceito pela API quando há sensor, gateway ou leitor com comunicação em rede. Um evento \textit{motion\_detected} pode iniciar uma janela de leitura; em seguida, a API disponibiliza o comando \textit{start\_reading} e recebe o evento \textit{tags\_read} com as identificações capturadas. Esse fluxo foi verificado por testes funcionais de software, sem validação física com esses dispositivos.

\input{figs/arquitetura_fluxo_api_rfid.tex}

\clearpage

\section{Cadastro de locais, leitores e itens}
\label{sec:cadastro-locais-leitores-itens}
...
\subsection{Operações de cadastro}
\label{sec:operacoes-cadastro}

As telas de configurações disponibilizam listagem e consulta dos locais, leitores e itens cadastrados. Para perfis autorizados a gerenciar cadastros, cada uma dessas entidades pode ser incluída, editada ou removida; os demais perfis permanecem com acesso de consulta. No cadastro de leitores, são informados o identificador de hardware, o local associado, o tipo de operação e os parâmetros de comunicação. No cadastro de itens, a tag é vinculada ao bem e aos seus locais lógico e físico. Dessa forma, as operações de cadastro mantêm os dados necessários para o processamento posterior das leituras RFID.

\section{Fluxo de eventos RFID}
\label{sec:fluxo-eventos-rfid}
...
\label{sec:auditoria-inconsistencias-log}
...
\subsection{Acesso e permissões}
\label{sec:acesso-permissoes}

O acesso à interface administrativa exige credenciais de usuário. Após a autenticação, o sistema disponibiliza as funções de acordo com o perfil e as permissões associadas: usuários autorizados podem gerenciar cadastros, acionar leitores, executar auditorias, resolver inconsistências e consultar o log operacional. A administração também dispõe de recursos para criar, editar, desativar ou remover usuários e para configurar as permissões atribuídas aos perfis técnicos. No backend, essas restrições são aplicadas às rotas correspondentes, de modo que a interface não seja a única barreira de controle.
...
\label{sec:resultados-validacao-funcional}
